---
title: Data Model
description: SQLAlchemy entities — Posts, Comments, Projects, Voice Threads and Turns, and Videos — and how they relate.
navigation.icon: i-lucide-database
---

# Data Model

Waywo uses SQLAlchemy ORM models mapped to a single SQLite database file (`waywo.db`). The core models form a hierarchy: a **Post** has many **Comments**, and each Comment can produce one or more **Projects**. Separately, **Voice Threads** contain **Voice Turns** for the voice chat feature.

```
WaywoPost (1) ──< WaywoComment (1) ──< WaywoProject
VoiceThread (1) ──< VoiceTurn
```
...
Represents a voice chat conversation thread.

| Column | Type | Description |
|---|---|---|
| `id` | `String(36)` (PK) | UUID, auto-generated |
| `title` | `Text` | Conversation title (auto-generated from first message, editable) |
| `system_prompt` | `Text` | Optional custom system prompt |
| `created_at` | `DateTime` | Record creation time |
| `updated_at` | `DateTime` | Auto-updates on thread or turn activity |

**Relationships:** Has many `VoiceTurn` records (cascade delete).

## VoiceTurn

Represents a single message (user or assistant) within a voice thread.

| Column | Type | Description |
|---|---|---|
| `id` | `Integer` (PK) | Auto-incrementing |
| `thread_id` | `String(36)` (FK) | References `voice_threads.id` (CASCADE delete) |
| `role` | `String(20)` | `"user"` or `"assistant"` |
| `text` | `Text` | Transcribed user speech or LLM response |
| `audio_duration_seconds` | `Float` | Duration of recorded or generated audio |
| `tts_voice` | `String(200)` | TTS voice used (assistant turns only) |
| `stt_raw_json` | `Text` | Raw STT response JSON (user turns only) |
| `token_count` | `Integer` | LLM token count (assistant turns only) |
| `llm_duration_ms` | `Integer` | LLM generation time in ms |
| `tts_duration_ms` | `Integer` | TTS generation time in ms |
| `stt_duration_ms` | `Integer` | STT processing time in ms |
| `created_at` | `DateTime` | Record creation time |

**Indexes:** `thread_id`.

Turns are ordered by `created_at` ascending within a thread. Creating a turn also bumps the parent thread's `updated_at`.
