...
description: High-level architecture of Waywo — from Hacker News data collection through AI processing to semantic search and RAG retrieval.
...
In addition to the data pipeline, Waywo includes a **Voice Mode** for conversational AI interaction via speech. This is a separate real-time system that uses WebSockets for bidirectional audio streaming.

```
Browser Mic → PCM → Backend WS → STT Service WS → Transcription
                                                        ↓
Browser ← WAV Audio ← TTS Service ← LLM Response ← LLM
```

Voice Mode uses three external GPU services (STT, LLM, TTS) and persists conversations as threads in the same SQLite database. See [Voice Mode](/voice-mode/overview) for full documentation.
