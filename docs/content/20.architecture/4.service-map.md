---
title: Service Map
description: Docker Compose service topology — every container, port mapping, purpose, and health check in the Waywo stack.
...
| **STT** | `STT_URL` | `http://192.168.5.96:8001` | NVIDIA Nemotron speech streaming | Speech-to-text (HTTP + WebSocket) |
| **TTS** | `TTS_URL` | `http://192.168.6.3:9000` | NVIDIA Magpie NIM | Text-to-speech synthesis |
| **Content Safety** | `CONTENT_SAFETY_URL` | `http://192.168.5.253:8085` | Nemotron-Content-Safety-Reasoning-4B | Prompt and response safety classification |
...
| `STT_URL` | `http://192.168.5.96:8001` | Speech-to-text service |
| `TTS_URL` | `http://192.168.6.3:9000` | Text-to-speech service |
| `CONTENT_SAFETY_URL` | `http://192.168.5.253:8085` | Content safety classification (vLLM) |
| `CONTENT_SAFETY_ENABLED` | `true` | Enable/disable safety checks |
