<template>
  ...
  >
    <div
      v-if="isOpen"
      class="fixed inset-0 z-30 bg-background/30 backdrop-blur-[2px] lg:hidden"
      @click="$emit('close')"
    />
  </Transition>
  ...
  >
    <div
      v-if="isOpen"
      class="fixed left-3 top-[4.5rem] z-40 w-72 max-h-[calc(100vh-6rem)] bg-background border rounded-xl shadow-lg flex flex-col overflow-hidden"
    >
      <!-- Header -->
